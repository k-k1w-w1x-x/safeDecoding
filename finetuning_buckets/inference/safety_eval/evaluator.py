    ...
    if accelerator.is_local_main_process:
        ...
        if save_path is not None:
            ...
            with open(save_path, 'w') as f:
                json.dump(log, f)
# wxk
def eval_safety_in_batch_by_safeDecoding(model, expert_model,prompt_style, tokenizer, num_prefix_tokens = 0, save_path = None, batch_size_per_device = 16,
                bench = 'hex-phi', evaluator = 'key_word',
                system_prompt = None, input_template = None, output_header = None, 
                max_new_tokens = 512, 
                do_sample = True, top_p = 0.9, temperature = 0.6, use_cache = True, top_k = 50,
                repetition_penalty = 1.0, length_penalty = 1.0, using_speculative = True,cmd_log_path=None,**kwargs):
    ...
    Generator = chat.SafeChat(model = model,expert_model=expert_model, prompt_style = prompt_style, tokenizer = tokenizer,
                         init_system_prompt = system_prompt)
    ...
    if cmd_log_path is None:
        ...
        with open(cmd_log_path, "a") as file:
